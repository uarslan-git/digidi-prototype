...
namespace RealisticEyeMovements {
	...
	public class EyeAndHeadAnimatorEditor : Editor
	{
		#region fields
			...
			SerializedProperty maxHeadPitchUpAngleProp;
			SerializedProperty maxHeadPitchDownAngleProp;
			SerializedProperty maxHeadYawAngleProp;
			...
			SerializedProperty useUpInsteadOfSpineAngleProp;
		...
		void DrawHeadConfiguration()
		{
			...
			if ( showHeadFoldout )
			{
				...
				if ( eyeAndHeadAnimator.headComponent.headControl != HeadComponent.HeadControl.None )
				{
					...
					EditorGUILayout.LabelField("Max head angles", EditorStyles.boldLabel);
					EditorGUI.indentLevel++;
					EditorGUILayout.Slider(maxHeadPitchUpAngleProp, 0, 120, new GUIContent( "Pitch up angle", "Max pitch up angle of the head"));
					EditorGUILayout.Slider(maxHeadPitchDownAngleProp, 0, 120, new GUIContent( "Pitch down angle", "Max pitch down angle of the head"));
					EditorGUILayout.Slider(maxHeadYawAngleProp, 0, 120, new GUIContent( "Yaw angle", "Max yaw angle of the head (look left/right around vertical axis)"));
					EditorGUI.indentLevel--;
					EditorGUILayout.Space();
					...
					EditorGUILayout.Slider(useUpInsteadOfSpineAngleProp, 0, 90, new GUIContent( "Up instead of spine angle", "If the spine is within this angle to the Up axis, when looking idly, character will look forward horizontally instead of perpendicular to the spine."));
				}
		...
		void InitVariables()
		{
			...
			{
				leftEyeFromAnimator = Utils.GetHumanoidBone(animator, HumanBodyBones.LeftEye);
				rightEyeFromAnimator = Utils.GetHumanoidBone(animator, HumanBodyBones.RightEye);
			}		
			
			canUseMecanimHeadBone = animator != null && animator.isHuman && Utils.GetHumanoidBone(animator, HumanBodyBones.Head) != null;
			canUseMecanimNeckBone = animator != null && animator.isHuman && Utils.GetHumanoidBone(animator, HumanBodyBones.Neck) != null;
		...
		void OnEnable()
		{
			...
			maxHeadPitchUpAngleProp = serializedObject.FindProperty("maxHeadPitchUpAngle");
			maxHeadPitchDownAngleProp = serializedObject.FindProperty("maxHeadPitchDownAngle");
			maxHeadYawAngleProp = serializedObject.FindProperty("maxHeadYawAngle");
			...
			useUpInsteadOfSpineAngleProp = serializedObject.FindProperty("useUpInsteadOfSpineAngle");
